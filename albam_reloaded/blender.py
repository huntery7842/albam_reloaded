...
class ALBAM_PT_CustomMaterialOptions(bpy.types.Panel):
    ...
    def draw(self, context):  # pragma: no cover
        ...
        for prop_name, _, _ in blender_registry.bpy_props.get(
            "material", []
        ):  # get unk_ properties for a material:'unk_01' 32835
...
class ALBAM_PT_CustomTextureOptions(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        for prop_name, _, _ in blender_registry.bpy_props.get(
            "texture", []
        ):  # prop_name :'unk_byte_1', _:1 -value
            layout.prop(tex, prop_name)

    @classmethod
    def poll(
        cls, context
    ):  # pragma: no cover #function is optional, used to check if the operator can run.
...
class AlbamImportOperator(bpy.types.Operator):
    ...
    def _import_file(self, **kwargs):
        ...
        obj = bpy.data.objects.new(
            name, None
        )  # Create a new object with the arc archive name, data = None
        obj.parent = parent
